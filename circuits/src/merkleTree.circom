...
template ExclusionMerkleTreeChecker(levels) {
    signal input lesserLeaf;
    signal input greaterLeaf;
    signal input root;
    ...
    hashers[0].left <== lesserLeaf;
    hashers[0].right <== greaterLeaf;

    for (var i = 0; i < levels-1; i++) {
        selectors[i] = DualMux();
        selectors[i].in[0] <== hashers[i].hash;
        selectors[i].in[1] <== pathElements[i];
        selectors[i].s <== pathIndices[i];
